...
for i in range(1,11):
    if os.path.exists(f"{thing}/keywords{i}"):
        for l in open(f"{thing}/keywords{i}").read().splitlines():
            n, s = l.split("\t")
            data[int(n)].append([m for m in re.findall(r"\('(.*?)', '(.*?)'\)", s)])
...
plt.title(f'Number of {thing} Articles Published Each Quarter from {min(years)} to {max(years)}')
...
plt.title('Distribution of Sentiment Values per Year')
